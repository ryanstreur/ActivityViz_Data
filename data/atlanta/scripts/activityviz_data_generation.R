...
# have_dl_dt = od_dt[!is.na(`HAVE_DL[Code]`) & `HAVE_DL[Code]`!= "",.(COUNT = .N), by = .(HAVE_DL, `HAVE_DL[Code]`)]
# setorder(have_dl_dt, "HAVE_DL[Code]")
# have_dl_dt[, `HAVE_DL[Code]`:=NULL]
...
fare_method_dt = fare_method_long_dt[,.(COUNT = .N,
...
use_service_dt = use_service_long_dt[,.(COUNT = .N,
